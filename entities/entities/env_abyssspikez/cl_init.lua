...
function ENT:DrawTranslucent()
	...
	local size = math.min(4, (-0.25 + delta) * 0.75)
	...
	if size == 1 then
		...
		icea = -0.8
		self.Rotation = self.Rotation + FrameTime() * 0
	end
...
function ENT:Initialize()
	...
	self:SetColor(Color(155, 125, 125, 255))
	self:SetMaterial("models/shadertest/shader2")
	--ExplosiveEffect(self:GetPos(), 48, 100, DMGTYPE_ICE)


	self:EmitSound("physics/concrete/boulder_impact_hard"..math.random(1, 3,2,4)..".wav", 100, math.random(160, 180))
	self:EmitSound("physics/concrete/boulder_impact_hard"..math.random(1, 3,2,4)..".wav", 100, math.random(160, 180))
	self:EmitSound("physics/concrete/boulder_impact_hard"..math.random(1, 3,2,4)..".wav", 100, math.random(160, 180))
