	...
	if !self.trg:IsValid() then
		...
		for _, ent in pairs(player.FindInSphere(self:GetPos(), 1548)) do
			...
			if WorldVisible(self:LocalToWorld(Vector(0, 0, 30)), ent:NearestPoint(self:LocalToWorld(Vector(0, 0, 30))))  then
				if target:IsValidLivingHuman() and target ~= self:GetOwner() then 
		...
		local direction = (targetpos - self:GetPos()):GetNormal() + (self.Bounces < 17 and VectorRand():GetNormal() or Vector(0,0,0))
		local phys = self:GetPhysicsObject()
		phys:SetVelocityInstantaneous(direction * 450 * math.Rand(1,2))
		self.trg = NULL
		self.NextHook = CurTime() + math.Rand(0.02,0.2)
	end
	...
	if ent:IsValid() then
		if (ent.BeingControlled or ent:IsValidLivingHuman()) and owner:IsPlayer() then
			if ent:IsValidLivingHuman() then
				...
				ent:TakeSpecialDamage(16, DMG_DIRECT, owner, self)
				self:Remove()
