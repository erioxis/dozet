...
function ENT:Think()
	...
	dmger = dmger and dmger:IsValid() and dmger:IsPlayer() and dmger or owner
	...
	if (owner) and not (dmger:GetStatus("rot")) and !owner:GetZombieClassTable().Skeletal and (dmger:GetProgress("parasite_prog") -(dmger:IsSkillActive(SKILL_PARASITOID_2) and 2 or 0)) < CurTime() then
		...
		if see then
			for _,ent in pairs(ents.FindInSphere(owner:GetPos(),55)) do
				if ent and ent ~= owner and ent:IsValidLivingZombie() and (ent:Team() == TEAM_UNDEAD or ent == dmger) then
					ent:TakeSpecialDamage((self.Damage or 10) * (flimsy and 2.5 or 1),DMG_ACID,dmger, self, nil, 0)
				end
			end
		end
	end
