...
function ENT:Think()
	...
	for _, ent1 in pairs(ents.FindInSphere((objectphys:GetPos() or self:GetPos()), (object:GetModelScale() or 1) * 35)) do
		if ent1:IsValid() and ent1:IsPlayer() and ent1:Team() == TEAM_UNDEAD and !table.HasValue(anti,object:GetClass()) and owner:Team() == TEAM_HUMAN and !ent1:GetZombieClassTable().Boss and !ent1:GetZombieClassTable().DemiBoss  then
	...
	elseif owner:KeyDown(IN_ATTACK2) and not owner:GetActiveWeapon().NoPropThrowing then
		...
		if owner:KeyDown(IN_WALK) then
			local xdiff = math.NormalizeAngle(self.StartX- math.Round((owner.InputMouseX or 0)/15)*15)
			local ydiff = math.NormalizeAngle(self.StartY - math.Round((owner.InputMouseY or 0)/15)*15)
			local sxdiff = math.Round(xdiff/15)*15 * FrameTime() * 24
			local sydiff = math.Round(ydiff/15)*15 * FrameTime() * 24
