...
function ENT:Hit(vHitPos, vHitNormal, hitent)
	...
	if hitent:IsValid() and not hitent:IsPlayer() or (hitent:IsPlayer() and hitent:Team() ~= TEAM_HUMAN) then
		hitent:TakeSpecialDamage(67 * (hitent.PhysicsDamageTakenMul or 1), DMG_GENERIC, owner, self)
