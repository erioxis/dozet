...
function EFFECT:Init(data)
	...
	local randmin, randmax = -1, 1
	local normal = (self.EndPos - self.StartPos)
	normal:Normalize()
	--[[for i = -5, self.EndPos:Distance(self.StartPos), 20 do
	...
	--[[for i=1, 120 do
		...
		local vel = VectorRand():GetNormal() * 560
		particle:SetDieTime(1.5)
		particle:SetColor(self.Color)
...
function EFFECT:Render()
	...
	local size = delta * 150
--[[render.SetMaterial(matBeam2)
	render.DrawBeam(startpos, endpos, size * 0.2, 1, 0, self.Color)
	...
	for i = 5, nlen * (1 - delta), 32 do
		local set = i - CurTime() * 250
		local spinbeamsize = (1 + delta) * (CurTime() * 0.015) 
		...
		render.DrawBeam(prevspinpos, spinbeampos, 4, texcoord + i, texcoord + 1 + i, Color(137, 236, 56, self.Color.a))
		render.DrawBeam(prevspinpos2, spinbeampos2, 4, texcoord + i, texcoord + 1 + i, Color(30, 95, 0, self.Color.a))
