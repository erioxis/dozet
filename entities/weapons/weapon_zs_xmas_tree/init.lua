...
function SWEP:SpawnGhost()
	...
	if owner and owner:IsValid() then
		owner:GiveStatus("ghost_xmas_tree")
	end
end
...
function SWEP:RemoveGhost()
	local owner = self:GetOwner()
	if owner and owner:IsValid() then
		owner:RemoveStatus("ghost_xmas_tree", false, true)
	end
end
...
function SWEP:PrimaryAttack()
	...
	local status = owner:GetStatus("ghost_xmas_tree")
	...
	local ent = ents.Create("prop_xmas_tree")
	if ent:IsValid() then
		ent:SetPos(pos)
		ent:SetAngles(ang)
		ent:Spawn()
