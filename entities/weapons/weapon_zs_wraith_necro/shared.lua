...
SWEP.PrintName = "Necrosis Wraith"
...
function SWEP:DoAlert()
	...
	for _, ent in pairs(player.FindInSphere(screampos, 222)) do
		if ent and ent:IsValidHuman() then
			local entearpos = ent:EyePos()
			local dist = screampos:Distance(entearpos)
			if dist <= 222 and TrueVisible(entearpos, screampos) then
				local power = (222 / dist - 1) * 2
				viewpunch(ent, power)
				for i=1, 5 do
					timer.Simple(0.15 * i, function() viewpunch(ent, power - i * 0.125) end)
				end
				if SERVER then
					local necro = ent:GetStatus('necrosis')
