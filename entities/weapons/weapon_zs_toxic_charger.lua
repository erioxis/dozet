			...
			for _, trace in ipairs(traces) do
				...
				if trace.HitWorld then
					...
					if ent and ent:IsValid() and not ent:IsProjectile() then
						...
						local hitents = 1
						for k,v in pairs(ents.FindInSphere(ent:GetPos(),90)) do
							if CLIENT then break end
							if v and v:IsValid() and !v:IsPlayer() then
								v:TakeDamage(math.random(45,90) * (self:IsChargeCritical() and 1.6 or 1) * hitents, owner, self)
							end
							hitents = hitents - 0.03
						end
...
function SWEP:Move(mv)
	...
	elseif charge > 0 then
		...
		mv:SetSideSpeed(mv:GetSideSpeed() * 0.7)

		local mul = 1 + charge * 1 + (self:IsChargeCritical() and 0.3 or 0)
...
function SWEP:PreDrawViewModel(vm)
	render.SetColorModulation(0, 0.294, 0.02)
end
