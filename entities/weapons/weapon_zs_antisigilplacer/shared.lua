SWEP.PrintName = "Anti Sigil Placer"
...
function SWEP:Holster()
	if SERVER then
		for _, ent in ipairs(ents.FindByClass("point_fakeasigil")) do
			ent:Remove()
		end
	end
...
function SWEP:PrimaryAttack()
	...
	if tr.HitWorld and tr.HitNormal.z >= 0.8 then
		table.insert(GAMEMODE.ProfilerNodesAnti, tr.HitPos)
...
function SWEP:SecondaryAttack()
	...
	local newpoints = {}
	for _, point in pairs(GAMEMODE.ProfilerNodesAnti) do
		if point:DistToSqr(tr.HitPos) > 4096 then
			table.insert(newpoints, point)
		end
	end
	GAMEMODE.ProfilerNodesAnti = newpoints
...
concommand.Add("zs_asigilplacer", function(sender)
	if CanPlace(sender) then
		sender:Give("weapon_zs_antisigilplacer")
	end
end)
end
...
function SWEP:RefreshSigils()
	...
	for _, point in pairs(GAMEMODE.ProfilerNodesAnti) do
		if istable(point) or isnumber(point) then continue end
		local ent = ents.Create("point_fakeasigil")
		if ent:IsValid() then
...
function ENT:DrawTranslucent()
	if MySelf:IsValid() then
		...
		if wep:IsValid() and wep:GetClass() == "weapon_zs_antisigilplacer" then
			cam.IgnoreZ(true)
			render.SetBlend(0.5)
			render.SetColorModulation(0, 0.569, 1)
...
scripted_ents.Register(ENT, "point_fakeasigil")
