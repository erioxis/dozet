...
SWEP.PrintName = "МАНТРА "
SWEP.Description = "ЭТО КОНЕЦ"
...
function SWEP:PrimaryAttack(pos)
	local owner = self:GetOwner()
	if not owner:IsValid() then return end
	if not self:CanPrimaryAttack() then return end
	if ( SERVER )   then
		local ent = ents.Create("projectile_magicwand2")
		local owner = self:GetOwner()
		if ent:IsValid() then
			ent:SetPos( pos or owner:GetShootPos() )
...
function SWEP:SecondaryAttack()
	...
	for i=1,5 do
		self:PrimaryAttack(pos + right*20*i-(right*60))
		self:PrimaryAttack(pos + up*20*i-(up*60))
	end
end