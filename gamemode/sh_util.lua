	...
	for _, ent in pairs(ents.FindInSphere(epicenter, radius)) do
		if ent:IsValid() then
			...
			if TrueVisibleFilters(epicenter, nearest, inflictor, attacker, ent)
				...
				ent:TakeSpecialDamage((((radius - nearest:Distance(epicenter)) / radius) * basedmg) * (ent:IsValidLivingHuman() and ent.ClanAvanguard and 0.44 or 1), damagetype, attacker, inflictor, nearest)
